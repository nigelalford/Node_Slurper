(function getEsriAMD() {
	...
	var fs = require('fs'),
		...
		//esriModules = require('./esri_3.8_modules.js');

	// App variables
	var file_url = 'http://js.arcgis.com/3.8amd/js/esri/'
		DOWNLOAD_DIR = './esri/';

	// Confirm, build, and download file to folder
	...
	var child = exec('mkdir esri ', function(err, stdout, stderr) {
		if (err === 'A subdirectory or file esri already exists') {
			console.log(' "esri" folder exists, I will place the files inside this folder...');
			//dlHTTP(file_url);
		} else dlHTTP(file_url);
	});
	...
	var dlHTTP = function(url) {
		//var file = fs.createWriteStream(DOWNLOAD_DIR + '_coremap.js');
		var file;

	for (var i = esriAMD.length -1; i >= 0; i--){
		file = fs.createWriteStream(DOWNLOAD_DIR + esriAMD[i]);
		...
		/*for (var i = esriAMD.length - 1; i >= 0; i--) {
			http.get(file_url + esriAMD[i], function(res) {
				console.log('in');
				res.on('data', function(data) {
					console.log('writing');
					file.write(data);
		...
		/*for (var i = esriModules - 1; i >= 0; i--) {
			http.get(file_url + esriModules[i], function(res) {
				console.log(file_url + res.statusCode);
				if(res.statusCode == '404'){
					console.log('a file was not found' + res.StatusCode)
				}
				res.on('data', function(data) {
					file.write(data);
				}).on('end', function() {
				...
				console.log('There was a problem accessing the url, here is the message: ' + e.message);
			});
		};*/
	};
}())

//module.exports = getEsriAMD;